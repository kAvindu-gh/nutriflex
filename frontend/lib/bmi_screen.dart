...
//ipv4 address
const kBaseUrl = 'http://xxx.xxx.xxx.xxx:8000';
...
class _BMIScreenState extends State<BMIScreen> {
  ...
  Widget _goalBtn(
          String label, String emoji, bool active, VoidCallback onTap) =>
    Expanded(
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
                color: active ? kGreen : Colors.transparent, width: 1.5),
            boxShadow: active
                ? [BoxShadow(
                    color: kGreen.withOpacity(0.18), blurRadius: 12)]
                : [],
          ),
    ...
    GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: AnimatedDefaultTextStyle(
          duration: const Duration(milliseconds: 250),
          style: TextStyle(
              color: active ? kGreen : Colors.white, fontSize: 14),
          child: Text(label),
        ),
      ),
    );
...
class _ResultsSection extends StatelessWidget {
  ...
  const _ResultsSection(
      {super.key, required this.result, required this.scaleKey});
  ...
  static Widget _card(
          {required Widget child, bool highlighted = false}) =>
    ClipRRect(
    ...
    return AnimatedBuilder(
      ...
      builder: (_, child) => Container(
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
                color: kGreen.withOpacity(0.25 * _glow.value),
                blurRadius: 10, spreadRadius: 0),
            BoxShadow(
                color: kGreen.withOpacity(0.35 * _glow.value),
                blurRadius: 22, spreadRadius: 2),
            BoxShadow(
                color: kGreen.withOpacity(0.12 * (1 - _glow.value)),
            ...
            child: Column(children: [
              ...
              if (widget.result.conditions.isNotEmpty) ...[
                ...
                Wrap(
                  ...
                  children: widget.result.conditions.map((c) =>
                    Container(
                      padding: const EdgeInsets.symmetric(
                          horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(
                        color: const Color.fromARGB(255, 255, 8, 0).withOpacity(0.15),
                        borderRadius: BorderRadius.circular(20),
                        border: Border.all(
              ...
              const SizedBox(height: 18),
              // Macros — values come from backend (condition-adjusted)
              Row(mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                _MacroStat(
                    '${widget.result.proteinG.toInt()}g', 'Protein'),
                Container(height: 36, width: 1, color: Colors.white12),
                _MacroStat(
                    '${widget.result.carbsG.toInt()}g', 'Carbs'),
                Container(height: 36, width: 1, color: Colors.white12),
                _MacroStat(
                    '${widget.result.fatG.toInt()}g', 'Fats'),
              ]),
            ]),
          ),
        ),
