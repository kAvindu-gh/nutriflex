...
class _BMIScreenState extends State<BMIScreen> {
  ...
  Future<void> _calculate() async {
    ...
    if (h > 250) {
      setState(() => _error = 'Height must be 250 cm or under.');
      return;
    }
    if (w > 300) {
      setState(() => _error = 'Weight must be 300 kg or under.');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final username = user?.displayName?.trim().isNotEmpty == true
        ? user!.displayName!
        : user?.email;
    final userText = username != null ? 'Hello, $username' : 'Not logged in';
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: _inputField('Height (cm)', _hCtrl, Icons.straighten),
                ),
            ...
            Row(
              children: [
                Expanded(
                  child: _inputField('Age', _aCtrl, Icons.cake_outlined),
                ),
                const SizedBox(width: 14),
                Expanded(
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                Text(
                  '${widget.result.dailyCalories.toInt()}',
                  style: const TextStyle(
                    color: kGreen,
                    fontSize: 50,
                    fontWeight: FontWeight.bold,
                ...
                if (widget.result.conditions.isNotEmpty) ...[
                  ...
                  Wrap(
                    spacing: 8,
                    runSpacing: 6,
                    children: widget.result.conditions
                        .map(
                          (c) => Container(
                            padding: const EdgeInsets.symmetric(
                              horizontal: 10,
