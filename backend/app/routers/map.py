...
async def get_nearby_stores(req: NearbyStoresRequest):
    """
    Get real nearby grocery/supermarket stores via Geoapify Places API.
    Falls back to hardcoded stores if API key is missing or call fails.
    """
    try:
...
async def geocode_address(address: str = Query(..., description="Address to geocode")):
    """Convert a text address to lat/lng using Nominatim (free, no key needed)."""
...
async def place_order(user_id: str, req: PlaceOrderRequest):
    """Place an order — saves to Firestore and clears the user's cart."""
    try:
